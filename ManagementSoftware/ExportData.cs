...
namespace ManagementSoftware
{
    public class ExportData
    {
        ...
        //
        public void ExportResultToPdf(List<Result>? list)
        {
            if (list == null || list.Count <= 0)
            {
                ...
                using (SaveFileDialog sfd = new SaveFileDialog() { Filter = "PDF Files|*.pdf", Title = "Save a PDF File" })
                {
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        try
                        {
                            ...
                            PdfFont font = PdfFontFactory.CreateFont(StandardFonts.TIMES_ROMAN, PdfEncodings.UTF8,pdfDoc);

                            doc.SetFont(font);


                            doc.Add(new Paragraph("THỐNG KÊ MẺ NẠP").SetFontSize(20).SetBold().SetTextAlignment(TextAlignment.CENTER).SetFont(font));



                            Table table = new Table(UnitValue.CreatePercentArray(new float[] { 11f, 14f, 20,20,14f,11f }));
